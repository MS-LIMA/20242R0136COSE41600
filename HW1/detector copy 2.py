...
voxel_buffers_list = compute_static_voxels(pcd_buffers=pcd_buffers,
                          ...
                          distance_thresholds=[(0,40),
                                                (40,80),
                                                (80,999)],
                          matching_counts=[25, 15, 15])

distance_thresholds = [(0, 20), (20, 40), (40, 999)]    
eps_thresholds = [0.2, 0.3, 0.4]                        
min_points_thresholds = [10, 5, 3]                      
...
for pcd_target in pcd_targets:
    ...
    for voxel_grid in voxel_buffers_list:
        mask_temp = np.array(voxel_grid.check_if_included(numpy_to_v3v(points)))
        mask = np.logical_or(mask, mask_temp)
    points_filtered = points[~mask]
    points_org = points[mask]
    bboxes_3d = []
    for dist_thres, eps, min_points in zip(distance_thresholds, eps_thresholds, min_points_thresholds):
